...
サービス提供時間を **6:00〜27:00 JST（21時間）** に限定し、夜間3時間はEC2を停止。
...
**コスト効果**: EC2 稼働を 87.5%（21/24h）に抑制。パブリック IPv4 アドレス課金も停止中は発生しない。
...
| バックアップ種別 | 手段 | 頻度 | 保管先 |
|---|---|---|---|
| **データバックアップ** (SQLiteファイル) | EventBridge → SSM Run Command → S3 cp | 毎日 23:30 JST | Amazon S3 |
| **システムバックアップ** (EBSスナップショット) | Amazon DLM | 毎週日曜 3:30 JST | EBSスナップショット |
